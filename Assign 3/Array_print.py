...
def run_experiments(dictionary, dict_name, print_cleaned=False):
    ...
    if print_cleaned:
        print("\nSample cleaned training texts (first 20, truncated to 200 chars):")
        for i, txt in enumerate(X_train_text[:20]):
            print(f"[train {i}] {txt[:200]}{'...' if len(txt) > 200 else ''}")
        print("\nSample cleaned test texts (first 20, truncated to 200 chars):")
        for i, txt in enumerate(X_test_text[:20]):
            print(f"[test  {i}] {txt[:200]}{'...' if len(txt) > 200 else ''}")

    vectorizer = CountVectorizer(vocabulary=dictionary, lowercase=False, token_pattern=r"(?u)\b\w+\b")
...
if __name__ == "__main__":
    # Run experiments; set print_cleaned=True to see samples of cleaned arrays
    run_experiments(load_dictionary('keywords769.txt'), 'keywords769.txt', print_cleaned=True)
    run_experiments(load_dictionary('keywords35.txt'), 'keywords35.txt', print_cleaned=False)
